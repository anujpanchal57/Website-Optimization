...
var resizePizzas = function(size) {
  ...
  // Refactored the code below to avoid Forced Synchronous Layout
...
// Moved document.document.querySelectorAll(".randomPizzaContainer"); outside the for loop
// Changed it to document.getElementsByClassName from document.querySelectorAll
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer");

// Moved randomPizzas.length to a local variable
...
// Moved the variable var pizzasDiv = document.getElementById("randomPizzas"); out of the for loop
...
// Changed from animationReadyCheck to updatePositions in the code below
...
//Took this variable outside the for loop
  var sine = (document.body.scrollTop / 1250);

  //I've set the target to 5 and only there are 5 unique phases for each scroll
  var constArray = [];
  for (var i = 0; i < 5; i++) {
    constArray.push(Math.sin(sine + i));
  }

// Declared the var phase outside the for loop to prevent it from being created each iteration
  var phase;

//Repositioned pizzas, set the target to window.items.length (for the number of pizzas which are calculated below)
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var rows = 6;
  var totalPizzas = 36;
  // Declared the variable var elem outside the loop to prevent it being created for each iteration
  var elem;
  // Also declared variable movingPizzas outside the for loop to prevent DOM call on every iteration
  // Replace "querySelector" with getElementById
  var movingPizzas = document.getElementById('movingPizzas1');
  // calculates the cols and rows based on the browser's window size
  ...
  // Moved window.items here to stop updatePositions from re-defining items on every scrolling event
